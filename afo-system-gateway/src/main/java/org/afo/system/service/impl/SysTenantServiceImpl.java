...
import org.afo.common.redis.utils.CacheUtils;
import org.afo.common.tenant.core.TenantEntity;
import org.afo.common.tenant.helper.TenantHelper;
import org.afo.system.domain.*;
import org.afo.system.domain.bo.SysTenantBo;
import org.afo.system.domain.vo.SysTenantVo;
import org.afo.system.mapper.*;
import org.afo.system.service.ISysTenantService;
...
public class SysTenantServiceImpl implements ISysTenantService {
    ...
    public SysTenantVo queryByTenantId(String tenantId) {
        // 绕过 selectVoOne (内部 selectOne(wrapper, throwEx) 有 MyBatis 一级缓存问题)
        SysTenant entity = baseMapper.selectOne(new LambdaQueryWrapper<SysTenant>().eq(SysTenant::getTenantId, tenantId));
    ...
    public Boolean insertByBo(SysTenantBo bo) {
        ...
        user.setNickName(StringUtils.blankToDefault(bo.getNickName(), bo.getUsername()));
        user.setPhonenumber(StringUtils.blankToDefault(bo.getPhonenumber(), bo.getContactPhone()));
    ...
    public Boolean updateByBo(SysTenantBo bo) {
        SysTenant original = baseMapper.selectById(bo.getId());
        SysTenant tenant = MapstructUtils.convert(bo, SysTenant.class);
        tenant.setTenantId(null);
        boolean updated = baseMapper.updateById(tenant) > 0;
        if (updated
            && original != null
            && bo.getPackageId() != null
            && !Objects.equals(original.getPackageId(), bo.getPackageId())) {
            syncTenantPackage(original.getTenantId(), bo.getPackageId());
        }
        return updated;
    }
