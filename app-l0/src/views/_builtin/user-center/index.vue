...
import { fetchLeaveTenant } from '@/service/api';
  ...
  <div class="flex gap-16px">
    ...
    <NCard title="个人信息" class="w-360px shadow-sm">
      <div class="flex-x-center flex-wrap gap-24px">
        ...
        <NPopconfirm v-if="userInfo.user?.userId !== 1" @positive-click="leaveTenant">
          <template #trigger>
            <NButton type="error" ghost :loading="btnLoading">
              <template #icon>
                <SvgIcon icon="material-symbols:exit-to-app-rounded" class="size-20px" />
              </template>
              退出当前企业
            </NButton>
          </template>
