...
import { fetchGetDeptTree, fetchGetUserList, fetchRemoveUserFromTenant, fetchUpdateUserStatus } from '@/service/api/system';
    ...
    columns: () => [
      {
        ...
        align: 'center',
        width: 48,
        disabled: row => Boolean(row.tenantCreator)
      },
      {
        key: 'userName',
      ...
      },
      {
        ...
        render: row => {
          ...
          const removeTenantBtn = () => {
            return (
              <ButtonIcon
                ...
                tooltipContent="移出企业"
                popconfirmContent={`确认将用户 ${row.userName} 移出当前企业吗？该操作不会删除账号。`}
                onPositiveClick={() => handleRemoveFromTenant(row.userId)}
              />
            );
          };
          ...
          if (hasAuth('system:user:remove') && !row.tenantCreator) buttons.push(removeTenantBtn());
...
async function handleBatchDelete() {
  const { error } = await fetchRemoveUserFromTenant(checkedRowKeys.value);
  if (error) return;
  onBatchDeleted();
}

async function handleRemoveFromTenant(userId: CommonType.IdType) {
  const { error } = await fetchRemoveUserFromTenant([userId]);
  if (error) return;
  onDeleted();
}
    ...
    <div class="h-full flex-col-stretch gap-12px overflow-hidden lt-sm:overflow-auto">
      ...
      <NCard :title="$t('page.system.user.title')" :bordered="false" size="small" class="card-wrapper sm:flex-1-hidden">
        <template #header-extra>
          ...
          >
            <template #after>
              <NPopconfirm v-if="hasAuth('system:user:remove')" @positive-click="handleBatchDelete">
                <template #trigger>
                  <NButton size="small" ghost type="error" :disabled="checkedRowKeys.length === 0">
                    <template #icon>
                      <SvgIcon icon="material-symbols:person-remove-outline" class="text-icon" />
                    </template>
                    批量移出企业
                  </NButton>
                </template>
                确认将选中用户移出当前企业吗？该操作不会删除账号。
              </NPopconfirm>
