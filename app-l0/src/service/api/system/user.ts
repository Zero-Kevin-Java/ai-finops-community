...
/** 将用户移出当前企业 */
export function fetchRemoveUserFromTenant(userIds: CommonType.IdType[]) {
  return request<boolean>({
    url: `/system/user/tenant/${userIds.join(',')}`,
    method: 'delete'
  });
}
...
/** 获取当前用户可切换租户列表 */
export function fetchGetUserTenantList() {
  return request<Api.System.UserTenant[]>({
    url: '/system/user/tenant/list',
    method: 'get'
  });
}

/** 当前用户切换租户 */
export function fetchSwitchUserTenant(tenantId: CommonType.IdType) {
