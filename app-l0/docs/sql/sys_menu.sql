...
-- IFrame 类型
UPDATE `sys_menu` SET `component` = 'FrameView', `query_param` = '{"url":"https://gitee.com/weiling---shanghai---data/ai-finops"}', `is_frame` = 2, `icon` = 'carbon:cloud-service-management', `menu_name` = 'AI-FinOps' WHERE `menu_id` = 117;
...
-- 外链类型
UPDATE `sys_menu` SET `menu_name` = 'AI-FinOps', `order_num` = 100, `path` = 'https://gitee.com/weiling---shanghai---data/ai-finops', `component` = 'FrameView', `icon` = 'local-icon-gitee', `remark` = 'AI-FinOps 仓库地址' WHERE `menu_id` = 4;
INSERT INTO `sys_menu` (`menu_id`, `menu_name`, `parent_id`, `order_num`, `path`, `component`, `query_param`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_dept`, `create_by`, `create_time`, `update_by`, `update_time`, `remark`) VALUES (7, 'afo', 0, 100, 'https://gitee.com/weiling---shanghai---data/ai-finops', 'FrameView', '', 0, 0, 'M', '0', '0', '', 'mdi:github', 103, 1, sysdate(), null, null, 'afo 仓库地址') ON DUPLICATE KEY UPDATE `update_time` = sysdate();
INSERT INTO `sys_menu` (`menu_id`, `menu_name`, `parent_id`, `order_num`, `path`, `component`, `query_param`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_dept`, `create_by`, `create_time`, `update_by`, `update_time`, `remark`) VALUES (8, 'AI-FinOps', 0, 100, 'https://gitee.com/weiling---shanghai---data/ai-finops', 'FrameView', '', 0, 0, 'M', '0', '0', '', 'local-icon-gitee', 103, 1, sysdate(), null, null, 'AI-FinOps 仓库地址') ON DUPLICATE KEY UPDATE `update_time` = sysdate();
